...
namespace VoxelGameApplication
{
    ...
    {
        private static float[][] grad = new float[][]
        {
            new[] {1.0f,1.0f,0.0f}, new[]{-1.0f,1.0f,0.0f}, new[]{1.0f,-1.0f,0.0f}, new[]{-1.0f,-1.0f,0.0f},
            new[] {1.0f,0.0f,1.0f}, new[] {-1.0f,0.0f,1.0f}, new[] {1.0f,0.0f,-1.0f}, new[] {-1.0f,0.0f,-1.0f},
            new[] {0.0f,1.0f,1.0f}, new[] {0.0f,-1.0f,1.0f}, new[] {0.0f,1.0f,-1.0f}, new[] {0.0f,-1.0f,-1.0f}
        };
        ...
        { 
            ...
            66, 215, 61, 156, 180, 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 
        ...
        private static float dot(float x, float y, float z, float[] g)
        {
            return x * g[0] + y * g[1] + z * g[2];
        }

        public static float GenerateSingleNoiseValue(float xin, float yin, float zin)
        {
        ...
        public static float GenerateSimplexNoiseWithOctaves(int octaves, float x, float y, float z)
        {
            float value = 0.0f;
            int i;
            for (i = 0; i < octaves; i++)
            {
                value += GenerateSingleNoiseValue(
                    x * (float)Math.Pow(2, i),
                    y * (float)Math.Pow(2, i),
                    z * (float)Math.Pow(2, i)
                );
            }
            return value;
        }
